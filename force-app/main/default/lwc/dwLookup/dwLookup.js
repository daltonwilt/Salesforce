...
// Apex Imports
import lookup from '@salesforce/apex/GenericService.lookup';
import getRecentlyCreatedRecord from '@salesforce/apex/GenericService.getRecentlyCreatedRecord';

const DELAY = 10;

export default class DwLookup extends NavigationMixin(LightningElement) {
    @api name;
    @api label;
    ...
    connectedCallback() {
        ...
        if(this.object.includes('__c')){
            let obj = this.object.substring(0, this.object.length-3);
            this.objectLabel = obj.replaceAll('_',' ');
        }else{
            this.objectLabel = this.object;
        }
    ...
    handleInputChange(event){
        ...
        this.delayTimeout = setTimeout(() => {
            lookup({ objectAPI: this.object, fields: this.fields, term: term }).then(result => {
